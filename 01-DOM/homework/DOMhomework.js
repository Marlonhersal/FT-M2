...
function buildToDo(objeto, index) {
  // Tu código acá:
  let toDoShell = document.createElement('div');
  toDoShell.className = "toDoShell";
  let toDoText = document.createElement("span");
  toDoText.innerHTML = objeto.description
  toDoText.addEventListener("click",(x)=>{
    completeToDo(x);
  })
  toDoText.id = index
  if(objeto.complete){
...
function buildToDos(toDos) {
  // Tu código acá:
  let contador = 0;
  let array = toDos.map((x)=>{
    
    return buildToDo(x,contador++)
  });
...
function displayToDos() {
  ...
  let arrayDivs = buildToDos(toDoItems);
  for(let i = 0; i < arrayDivs.length; i++){ 
    toDoContainer.appendChild(arrayDivs[i])
  }
}
...
function addToDo() {
  ...
  let entrada = document.querySelector("#toDoInput");
  let toDo = new ToDo(entrada.value);
  toDoItems.push(toDo);
  entrada.value = "";
  displayToDos()
}
